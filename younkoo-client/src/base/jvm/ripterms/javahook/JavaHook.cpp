...
static void common_detour(Ripterms::JavaHook::HotSpot::frame* frame, java_hotspot::java_thread* thread, bool* cancel);
...
struct HookedMethod
{
	java_hotspot::method* method = nullptr;
...
#include <base/sdk/hook/HookUtils.h>
	...
	static int runonce = []()->int
		{
			...
			JVM::get().jvmti->AddCapabilities(&capabilities);
			return 0;
		}();
		if (!methodID || !detour)
			return false;
		...
		method = HookUtils::GenericResolve(methodID);
		...
		LOG("placed i2i hook at: " << (void*)target);
		...
		if (matches == sizeof(pattern))
		{
			for (uint8_t* curr2 = curr; curr2 > curr - 100; --curr2)
			{
				...
				if (matches2 == sizeof(pattern2))
					Ripterms::JavaHook::HotSpot::frame::locals_offset = *(char*)(curr2 + 3);
...
void common_detour(Ripterms::JavaHook::HotSpot::frame* frame, java_hotspot::java_thread* thread, bool* cancel)
{
	if (!(*(void**)thread->get_jni_environment()) || thread->get_thread_state() != _thread_in_Java || !JVM::get().Env) return;
	for (HookedMethod& hk : hooked_methods)
	{
...
void** Ripterms::JavaHook::HotSpot::frame::get_locals()
{
	if (!this) return nullptr;
	return *(void***)((uint8_t*)this + locals_offset); //48 on java8, 56 on java17
}

java_hotspot::method* Ripterms::JavaHook::HotSpot::frame::get_method()
{
	return *(java_hotspot::method**)((uint8_t*)this - 24);
}
