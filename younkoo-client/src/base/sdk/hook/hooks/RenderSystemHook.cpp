...
#include <wrapper/versions/1_20_1/net/minecraft/client/renderer/GameRenderer.h>
#include <wrapper/versions/1_20_1/com/mojang/blaze3d/vertex/PoseStack.h>
...
void GameRendererHook::hook(const HookManagerData& container)
{
	if (SRGParser::get().GetVersion() == Versions::FORGE_1_18_1) {
		...
		LOG("length :" << bytecodes_length);
		LOG("bytecode addr " << (void*)const_method->get_bytecode_start());
		...
		auto return_register_finalizer_list = method_block | std::views::filter([](const auto& bytecode_info) {
			return bytecode_info.opcode == java_runtime::bytecodes::_fast_aldc_w;
			});

		LOG(" return_register_finalizer_list.size() :" << std::ranges::distance(return_register_finalizer_list));
		const auto& opcode = *std::ranges::prev(std::ranges::end(return_register_finalizer_list));
		LOG("setting bytecode on index " << opcode.index << " whose original byte is " << (int)opcode.opcode << " method : " << method);
		...
		jvm_break_points::set_breakpoint_with_original_code(method, opcode.index, static_cast<std::uint8_t>(opcode.opcode), [mid](break_point_info* bp) -> void
			{
				...
				HookUtils::o_thread_map[bp->java_thread] = static_cast<JavaThreadState>(orginal_state);
					...
					std::call_once(flag, [bp] {
						{
							...
							for (auto& entry : entries)
							{
								...
								if (entry.name == "matrix4f")
								{
									...
									LOG("Found matrix4f_4f_slot in " << matrix4f_4f_slot);
								}

								if (entry.name == "p_109092_")
								{
									poseStack_slot = entry.slot;
									LOG("Found poseStack_slot in " << poseStack_slot);
								}
							}
						}
		...
		jclass klass = JNI::find_class(V1_20_1::GameRenderer::get_name());


		// Prevent method inlining to avoid crash with "error: unhandled bytecode".
		{
			auto mid = (jmethodID)V1_20_1::GameRenderer::static_obj().render;
			const auto method = *reinterpret_cast<java_hotspot::method**>(mid);
			//methods_dont_compile.push_back(method);
		}
		...
		JVM::get().jvmti->RetransformClasses(1, &klass);
		auto mid = (jmethodID)V1_20_1::GameRenderer::static_obj().renderLevel;
		auto method = *reinterpret_cast<java_hotspot::method**>(mid);
		...
		jvm_break_points::set_breakpoint_with_original_code(method, opcode.index, static_cast<std::uint8_t>(opcode.opcode), [mid](break_point_info* bp) -> void
			{
				...
				V1_20_1::PoseStack stack(poseStack, true);
				V1_20_1::Matrix4f projectionMatrix(matrix4f, true);
				V1_20_1::GameRenderer gameRenderer(gameRender_obj, true);

				static auto fill_matrix_with_matrix4f = [](Math::Matrix& matrix_struct, V1_20_1::Matrix4f& matrix_obj) {
					matrix_struct.m00 = matrix_obj.m00.get();  matrix_struct.m01 = matrix_obj.m01.get(); matrix_struct.m02 = matrix_obj.m02.get(); matrix_struct.m03 = matrix_obj.m03.get();
