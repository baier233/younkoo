...
#include "resources/maps/fabric1204.h"



void SRGParser::Init(const unsigned char* srgBytes, size_t size, bool Reverse, bool compressed)
{
	std::string srgData(reinterpret_cast<const char*>(srgBytes), size);
	if (compressed)
	{
		srgData = gzip::decompress(reinterpret_cast<const char*>(srgBytes), size);
	}
...
void SRGParser::SetVersion(Versions ver)
{
	this->version = ver;
	switch (ver) {
	case Versions::FORGE_1_8_9: this->Init(forge189, forge189_size, false, true); break;
		//case Versions::VANILLA_1_8_9:this->Init(vanilla189, vanilla189_size, false, true); break;
	case Versions::FORGE_1_18_1: this->Init(forge1181, forge1181_size, true, true); break;
	case Versions::FORGE_1_20_1: this->Init(forge1201, forge1201_size, true, true); break;
	case Versions::FORGE_1_12_2: this->Init(forge112, forge112_size, true, true); break;
	case Versions::FABRIC_1_20_4: this->Init(fabric1204, fabric1204_size, true, true); break;
		//case Versions::VANILLA_1_12_2: this->Init(vanilla112, vanilla112_size, true, false); break;

		//case Versions::BADLION_1_8_9:this->Init(vanilla189, vanilla189_size, false, false);
	default:
		break;
	}
...
#ifdef LOG
		LOG("WrapperField:" << originalFieldName << " Obfuscator:" << obfuscatedName);
#endif
...
#ifdef LOG
		LOG("WrapperMethod:" << originalClassName << " Desc:" << methodDesc << " Obfuscator:" << obfuscatedMethodName << " ObfuscatorDesc:" << obfuscatedMethod.second);
#endif
...
#ifdef LOG
		LOG("WrapperClass:" << originalClassName << " Obfuscator:" << classMappings[originalClassName]);
#endif
